...
class PostServiceTests {
    ...
    fun `viewing a post increases view count`() = runBlocking {
        val post = service.createPost("hello", null, null, "u1")
    ...
    fun `delete marks post and comment`() = runBlocking {
        val post = service.createPost("hello", null, null, "u1")
        val comment = service.addComment(post.id, "hi", "u2")!!
        val child = service.addComment(post.id, "reply", "u3", comment.id)!!
        service.deleteComment(post.id, child.id, comment.id)
